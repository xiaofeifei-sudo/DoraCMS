...
const ContentMessageController = {
  ...
  // 渲染child
  async renderChildren(ctx, parentId) {
    ...
    if (!_.isEmpty(childMessages)) {
      for (let childMessagesKey in childMessages) {
        childMessages = childMessages.concat(this.renderChildren(childMessagesKey._id))
      }
    }
